...
const { validateAndCalculateSectionDims } = require('./helpers');
const { config } = require('./config');
const { fitOptions } = require('./enums');

class PdfService {
    async setUpPdf() {
        this.pdfDoc = await PDFDocument.create();
        this.pdfDoc.registerFontkit(fontkit)
        this.font = await this.pdfDoc.embedFont(fs.readFileSync('./assets/Courgette Regular Mac.ttf'));
    }
    ...
    async drawImage(options) {
        try {
            const { file, fileName, maxWidth, maxHeight, widthScale, heightScale, fit, side, border, section } = options;
            ...
            let image = await this.pdfDoc[fileExtension === 'jpg' ? 'embedJpg' : 'embedPng'](file);
            ...
            if (fit) {
                ...
                if (fit === fitOptions.fillAndCrop) {
                    ...
                    const test = await sharp(file)
                        .resize(resizeWidth, resizeHeight) // maintains aspect ratio
                        .extract(cropOptions)
                        .toFile(croppedFilePath);
                    image = await this.pdfDoc[fileExtension === 'jpg' ? 'embedJpg' : 'embedPng'](fs.readFileSync(croppedFilePath));
